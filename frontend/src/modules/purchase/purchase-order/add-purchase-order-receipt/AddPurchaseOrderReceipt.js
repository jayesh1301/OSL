...
import { addPurchaseOrder } from "../../../../lib/api-purchase";
...
function AddPurchaseOrderReceipt() {
  ...
  const handleEdit = (row) => {
    ...
    const vehicleType = vehicleTypeOptions.find(
      (option) => option.vehicle_type_id === row.VehicleTypeId
    );
    const FromCutsomer = customerOptions.find(
      (option) => option.customer_id === row.FromcustomerID
    );
    const ToCutsomer = customerOptions.find(
      (option) => option.customer_id === row.TocustomerID
    );
    const state = states.find(
      (option) => option.statecode === row.statecode
    );

    const gstName = gstData.find(
  ...
  const handleSave = () => {
    ...
    const dataToSend = {
      ...
      billingAddressFlag: checkboxState ? 1 : 0,
    };

    addPurchaseOrder(dataToSend)
      .then(response => {
      ...
      .catch(error => {
        ...
        setTimeout(() => {
          navigate('/error-page'); // Replace with your actual error page
