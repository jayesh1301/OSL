...
import { addSaleInvoice, getGSTNumberAndState, getLorryReceiptById, getLorryReceiptList, getPODetailsByPOID } from '../../../../lib/api-salesInvoice1';
...
function AddSalesInvoice() {
  ...
  const handleAddButtonClick = async () => {
    try {
      // Generate a list of promises for fetching data for each selected LR ID
      ...
      // Flatten the array of records from all responses
      const newData = responses.flatMap((response) => response.data.records || []);
  
      // Map the new data to include the selected LR IDs
      const dataWithSelectedIds = newData.map(record => ({
        ...record,
        selectedLrIds // Add the selectedLrIds to each record
      }));
  
      // Update tableData with the new records including the selected IDs
      setTableData((prevTableData) => [...prevTableData, ...dataWithSelectedIds]);
  
      // Update LR count based on the number of selected IDs
      setLRCount(prevCount => prevCount + selectedLrIds.length);
  
      // Clear selected LR IDs
      setSelectedLrIds([]);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    if (!tableData || tableData.length === 0) {
      setSnackbarMessage('At least one LR entry is required.');
    ...
    const dataToSend = {
      ...
      BillDate: billDate ? dayjs(billDate).format('YYYY-MM-DD') : '',
    ...
    try {
      const response = await addSaleInvoice(dataToSend);
