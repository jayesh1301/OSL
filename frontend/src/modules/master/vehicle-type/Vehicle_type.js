...
import { getVehicleTypeList, deleteVehicleType, updateFlagStatus, getAllVehicleTypeList } from '../../../lib/api-master';
...
function Vehicle_type() {
  ...
  const getJsonData = async () => {
    ...
    // console.log("vt resp : ", response);

    const startIdx = paginationModel.page * paginationModel.pageSize;
    const jsonData = response.data.vehicletype.map((row, index) => ({
      sn: startIdx + index + 1,
      id: row.vt_id,
      vehicleType: row.vehicle_type,
      tyreQuantity: row.tyre_qty,
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if( selectedArticles.length === 0){
        response = await deleteVehicleType([deleteid])
        setDeleteid(null)
        getJsonData();
        setSelectedArticles([])
        
     }else if(!deleteid){
        response = await deleteVehicleType(selectedArticles)
     }
 
      if (response.status == 200) {
        let msg = "Vehicle Type already in use :";
        if (response.data.length > 0) {
          ...
          setConfirmmessage('Vehicle Type delete successfully');
          setConfirmationopen(true);
          setColor('success')
          getJsonData();
          setSelectedArticles([])
  ...
  const handleDelete = async () => {
    if (selectedArticles.length > 0) {
      ...
      setConfirmmessage('No Vehicle Type selected for deletion.');
      setConfirmationopen(true);
       setColor('warning')
    }
  };
  ...
  const ActionsCell = ({ id, status }) => {
    const handleEditClick = () => {
      navigate(`/Edit_vehicle-type/${id}`)
      // console.log("Edit button clicked for row with ID:", id);
    };

    const handleToggle = async () => {
      setIsLoading(true)
      const response = await updateFlagStatus({ id, status, name: "vehicle_type", colname: "vt_id" })
      if (response.status == 200) {
        getJsonData()
      }
    };
  ...
  const columns = [
    ...
    { field: 'vehicleType', headerName: 'Vehicle Type', flex: 1, headerClassName: classes.blueHeader, cellClassName: classes.columnBorder, resizable: true },
    { field: 'tyreQuantity', headerName: 'Tyre Quantity', flex: 1, headerClassName: classes.blueHeader, cellClassName: classes.columnBorder, resizable: true },
    {
      field: 'select',
      headerName: 'Select',
      flex: 1,
      renderCell: (params) => <Checkbox color='primary' onChange={(event) => handleSelectArticle(event, params.row.id)} />,
  ...
  const exportToExcel = async () => {
    setIsLoading(true)
      try {
          const allarticle = await getAllVehicleTypeList();
        const filteredRows = allarticle.data.map((item, index) => ({
              Srno: index + 1, 
              "Vehicle Type": item.vehicle_type,
              "Tyre Quantity": item.tyre_qty
          }));
          
          const worksheet = XLSX.utils.json_to_sheet(filteredRows);
      
          const workbook = XLSX.utils.book_new();
          XLSX.utils.book_append_sheet(workbook, worksheet, "VehicleType");
          XLSX.writeFile(workbook, "VehicleType.xlsx");
      } catch (error) {
  ...
  return (
    <>
      ...
      <div style={{ backgroundColor: '#f5f5f5', padding: '20px', fontFamily: 'Arial, sans-serif' }}>
        <div style={{ marginBottom: '20px', textAlign: 'center', fontSize: '24px', fontFamily: 'Poppins, sans-serif' }}>Vehicle Type List</div>
        ...
        {/* <div style={{ height: 790, width: '100%', marginTop: '20px' }}>
          <DataGrid
            rows={filteredRows}
            columns={columns}
            pageSize={5}
