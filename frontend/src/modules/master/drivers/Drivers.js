...
import { Box, Button, Checkbox, IconButton, Switch } from '@mui/material'; // Import Checkbox from Material-UI
...
import { getDriversList, deleteDriver, updateFlagStatus, getAllDriversList, getAllDriversserach, getAllDrivers } from '../../../lib/api-master';
...
function Drivers() {
  ...
  const getJsonData = async () => {
    try {
      ...
      const response = await getDriversList(
      ...
      const jsonData = response.data.driver.map((row, index) => ({
        id: row.driver_id,
        sn: startIdx + index + 1,
  ...
  const getJsonDataforserach = async () => {
    setIsLoading(true)
    const response = await getAllDriversserach(paginationModel.page,
      paginationModel.pageSize,
      searchModel);
    
    console.log("dri resp : ", response);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if( selectedArticles.length === 0){
        response = await deleteDriver([deleteid])
        setDeleteid(null)
        getJsonData();
        setSelectedArticles([])
        
     }else if(!deleteid){
        response = await deleteDriver(selectedArticles)
     }
      if (response.status == 200) {
        let msg = "Driver already in use:";
  ...
  const handleDelete = async () => {
    if (selectedArticles.length > 0) {
      ...
      setConfirmmessage('No Driver selected for deletion.');
      setConfirmationopen(true);
       setColor('warning')
    }
  };
  ...
  const columns = [
    ...
    { field: 'name', headerName: 'Name', flex: 0.7, headerClassName: classes.blueHeader, cellClassName: classes.columnBorder, resizable: true },
    { field: 'address', headerName: 'Address', flex: 1.5, headerClassName: classes.blueHeader, cellClassName: classes.columnBorder, resizable: true },
    { field: 'license', headerName: 'License', flex: 0.5, headerClassName: classes.blueHeader, cellClassName: classes.columnBorder, resizable: true },

    { field: 'phone', headerName: 'Mobile Number', flex: 1, headerClassName: classes.blueHeader, cellClassName: classes.columnBorder, resizable: true },
    {
      field: 'checkbox', // New field for checkboxes
      headerName: 'Select',
      flex: 0.3,
      renderCell: (params) => (
        <Checkbox
  ...
  const ActionsCell = ({ id, status }) => {
    ...
    const handleToggle = async () => {
      setIsLoading(true)
      const respnse = await updateFlagStatus({ id, status, name: "driver", colname: "driver_id" })
  ...
  const exportToExcel = async () => {
    setIsLoading(true)
      try {
          const allarticle = await getAllDrivers();
  ...
  return (
    <>
      ...
      <div style={{ backgroundColor: '#f5f5f5', padding: '20px', fontFamily: 'Arial, sans-serif' }}>
        <div style={{ marginBottom: '20px', textAlign: 'center', fontSize: '24px', fontFamily: 'Poppins, sans-serif' }}>Driver List</div>
