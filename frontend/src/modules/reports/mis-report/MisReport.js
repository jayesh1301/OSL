...
import { getmisreports } from "../../../lib/api-reports";
...
function MisReport() {
  ...
  const fetchDataWithPagination = async () => {
    ...
    try {
      ...
      const misReport = response.data.misReport;
      setRows(misReport);
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const formattedData = misReport.map((item, index) => {
        return {
          "Sr No": index + 1,
          ...
          "Memo_No": item.Memo_No == "undefined" || item.Memo_No == null ? "" : item.Memo_No ?? "",
      ...
      const fileName = `MISReport_export_export_${formattedDate}_${formattedTime}.xlsx`;
          ...
          <TableBody>
            {rows.map((row, index) => (
              <TableRow key={row.id}>
                ...
                <TableCell className={classes.tableCell}>{ row.irBranch == "undefined" || row.irBranch == null ? "" : row.irBranch ?? ""}</TableCell>
                <TableCell className={classes.tableCell}>{ row.deliveryDate == "undefined" || row.deliveryDate == null ? "" : row.deliveryDate ?? ""}</TableCell>
